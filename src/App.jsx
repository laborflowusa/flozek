...
const CITIES = [
  ...
  { name:"Allentown, PA",         stconst GLOSSARY = [
...
export default function Flozek() {
  ...
  if (tab === "test") {
    ...
    return (
      <div style={app}><style>{CSS}</style>
        <div style={{ padding:"52px 22px 0" }}>
          <div style={{ fontFamily:"'Syne',sans-serif", fontSize:22, fontWeight:800, background:"linear-gradient(135if (tab === "brands") {
        ...
        <div style={{ padding:"52px 22px 0" }}>
          ...
          {learnTab === "academy" && (
            <div style={{ animation:"fadeUp 0.3s ease both" }}>
              <div style={{ display:"flex", gap:6, marginBottom:14, flexWrap:"wrap" }}>
                {LESSONS.map((l,i)=>(
                  <button key={i} className="btn" onClick={()=>setEduIdx(i)} style={{ padding:"6px 10px", background:eduIdx===i?`rgba(${l.color === "#00d4ff"?"0,212,255":l.color==="63d39e"?"99,211,158":"251,191,36"},0.15)`:"rgba(255,255,255,0.04)", border:`1px solid ${eduIdx===i?l.color+"40":"rgba(255,255,255,0.08)"}`, borderRadius:20, fontSize:10, color:eduIdx===i?l.color:"rgba(200,230,215,0.45)" }}>
                    {l.icon} {l.title}
                  </button>
                ))}
              </div>
